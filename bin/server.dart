...
Future<void> main(List<String> args) async {
  ...
  if (!logFile.existsSync()) {
    print('if you want to capture logs into file create a file named `socket.log`');
  }
...
void recordLog(HttpConnectionInfo? connection, String raw) {
  ...
  if (connection != null) {
    buffer.writeAll(
      ['Connection info:', '\tip: ${connection.remoteAddress.address}', '\tport: ${connection.remotePort}', ''],
      '\n',
    );
  } else {
    buffer.writeAll(
...
String asLogEntry(Map<String, dynamic> data) {
  ...
  buffer.writeln('Record Time: ${DateTime.fromMillisecondsSinceEpoch(data['time']).toLocal().toIso8601String()}');
